...
def root_fun(original_input, prompt_masks, temperature, repetition_penalty):
    ...
    values = get_values(original_input.input_ids).cpu().numpy()
   

    return priors, values, states


def rec_fun(states, token_ids, attention_masks, prompt_masks, temperature, repetition_penalty, rollout_size):
...
class BatchedMCTS():
    ...
    def dense_visit_counts(self):
        batch_token_ids = [[] for i in range(self._batch_size)]
        root_index = np.zeros(self._batch_size, dtype=np.int32)
        is_reach = root_index == -1
        ipdb.set_trace()
        while not is_reach.all():
            root_visit_counts = self._children_visits[self._batch_range, root_index, :]
            select_children = np.argmax(root_visit_counts, axis=1)
            ...
            for i in range(self._batch_size):
                tag = dense_visit_counts[i] == 0
                if not tag.all():
                    batch_token_ids[i].append(np.argmax(dense_visit_counts[i]))
                else:
                    batch_token_ids[i].append(50256)
            root_index = np.squeeze(self._children_index[self._batch_range, root_index, select_children])
            is_reach = root_index == -1
    ...
    def expand(self, node_indices, actions, next_node_index):
        ...
        # Evaluate nodes.
        (prior, values, next_states) = self._rec_fun(states, tokens_ids, attention_masks, prompt_masks, self._temperature, self._repetition_penalty, self.rollout_size)
...
def mcts_search(prompt_texts):
    ...
    MCTS = BatchedMCTS(root_fun, rec_fun, batch_size=args.batch_size, num_simulations=args.num_it, num_actions=vocab_size+1, num_sparse_actions=50, pb_c_init=args.c, temperature = args.temperature, alpha=args.alpha, penalty=args.penalty, rollout_size = args.rollout_size)
    ...
    ipdb.set_trace()
    original_input.input_ids = torch.cat((original_input.input_ids, torch.cuda.LongTensor(res_search)), dim=1)
...
if __name__ == "__main__":
    input = ["Jobs established Apple company which is", "Nick picked up an"]
    text = mcts_search(input)
    print(text)
